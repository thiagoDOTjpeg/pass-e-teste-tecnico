...
export function TodoList({ todos, isLoading, onToggle, onDeleteRequest, onEditRequest }: TodoListProps) {
    ...
    return (
        <div className="space-y-4">
            {todos.map((todo) => (
                <div key={todo.id} className="todo-item flex items-center justify-between rounded-lg border p-4 shadow-sm transition-all hover:bg-accent/5">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="mt-1 flex flex-wrap gap-2">
                                <Badge variant="outline" className="h-5 px-1.5 text-[12px] font-normal text-muted-foreground">
                                    Criada {formatDistanceToNow(new Date(todo.created_at), { addSuffix: true, locale: ptBR })}
                                </Badge>

                                {todo.updated_at !== todo.created_at && (
                                    <Badge variant="secondary" className="h-5 px-1.5 text-[12px] font-normal">
                    ...
                    <div className="flex">
                        <Button variant="ghost" size="sm" onClick={() => onEditRequest(todo)}>
                            Editar
                        </Button>
                        <Button
                            variant="ghost"
